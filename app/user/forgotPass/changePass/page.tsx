...
const Page = () => {
  ...
  const changePass = async () => {
    setError("");
    if (!email || !newPassword || !confirmPassword) { setError("Бүх талбарыг бөглөнө үү"); return; }
    ...
    <div className="min-h-screen bg-[#111] flex items-center justify-center px-4">
      <div className="w-full max-w-[400px] bg-white rounded-2xl shadow-2xl overflow-hidden">
        <div className="flex items-center border-b border-[#e5e5e5] px-6 py-4">
          <button
            onClick={() => router.push("/user/forgotPass/verifyCode")}
            className="text-[13px] text-[#888] hover:text-black transition-colors bg-transparent border-none cursor-pointer"
          >
            ← Буцах
          </button>
          <span className="mx-auto text-[14px] font-bold text-black">Нууц үг солих</span>
          <button
            onClick={() => router.push("/")}
        ...
        <div className="px-8 py-7">
          {success ? (
            <div className="text-center py-8">
              ...
              <p className="font-bold text-black text-[16px]">Нууц үг солигдлоо!</p>
              <p className="text-[13px] text-[#888] mt-1">Нэвтрэх хуудас руу шилжиж байна…</p>
            </div>
          ) : (
            <>
              {error && (
              ...
              <div className="flex flex-col gap-4 mb-5">
                ...
                <div>
                  <label className="block text-[12px] font-semibold text-[#444] mb-1.5">Шинэ нууц үг</label>
                  <input
                    type="password"
                    placeholder="••••••••"
                    value={newPassword}
                ...
                <div>
                  <label className="block text-[12px] font-semibold text-[#444] mb-1.5">Нууц үг давтах</label>
                  <input
                    type="password"
                    placeholder="••••••••"
              ...
              >
                {loading ? "Солж байна…" : "Нууц үг солих"}
              </button>
            </>
          )}
        </div>
