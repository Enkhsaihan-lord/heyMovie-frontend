...
const Page = () => {
  ...
  async function uploadVideo(file: File, title: string): Promise<string> {
    ...
    if (!initRes.ok) throw new Error(initData.error || "Видео upload амжилтгүй");
  ...
  async function createSeries() {
    ...
    if (!seriesInfo.name || !seriesInfo.description || !seriesInfo.seriesNumber) {
    ...
    try {
      const [imageUrl, videoUrl] = await Promise.all([
        ...
        videoFile ? uploadVideo(videoFile, seriesInfo.name) : Promise.resolve(undefined),
      ]);

      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/series/seriesPost`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ ...seriesInfo, image: imageUrl, videoUrl, subscription }),
      });
      const data = await res.json();
    ...
    <div className="min-h-screen bg-[#1F1F1E] flex items-center justify-center font-sans px-4 py-12">
      <div className="w-full max-w-[420px]">
        ...
        {success ? (
          ...
          <>
            <h1 className="text-[24px] font-bold text-[#F4F1DE] mb-1">Series нэмэх</h1>
            ...
            <div className="flex flex-col gap-4 mb-6">
              ...
              <div className="flex flex-col gap-2">
                <label className="text-[11px] uppercase tracking-widest text-[#A8A29E]">Series дугаар</label>
                <input
                  className={inputClass}
                  name="seriesNumber"
                  placeholder="1"
                  value={seriesInfo.seriesNumber}
                  onChange={handleInput}
                />
              </div>
