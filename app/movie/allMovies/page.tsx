...
const Page = () => {
    ...
    const fetchAll = async () => {
      try {
        const data = await fetch(
          `${process.env.NEXT_PUBLIC_API_URL}/api/allMovies`
        ).then((r) => r.json());
        setMovies(data);
        if (email) {
  ...
  const goToMovie = (id: string, subscription: string) => {
    if (subscription === "PREMIUM" && !userSub) {
      setShowSignInPrompt(true);
      return;
    }
    router.push(`/movie/movieDetail/${id}`);
  };
  ...
  return (
    <div className="min-h-screen bg-[#0d1117] text-white font-sans">
      ...
      <div className="pt-20 pb-16 px-4 md:px-10 lg:px-16 max-w-[1600px] mx-auto">
        ...
        {loading && (
          <p className="text-[#444] text-sm">Ачааллаж байна…</p>
        )}
        {error && <p className="text-red-400 text-sm">{error}</p>}

        {!loading && !error && movies.length === 0 && (
          <p className="text-[#6b7280] text-sm">Кино байхгүй байна</p>
        )}
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-3 md:gap-4">
          {movies.map((movie) => (
            <div
              key={movie.id}
              onClick={() => goToMovie(movie.id, movie.subscription)}
            ...
            >
              <div className="relative rounded-xl md:rounded-2xl overflow-hidden shadow-lg shadow-black/40">
                ...
                <div className="absolute top-2 left-2">
                  <span
                    className={`text-[9px] font-bold uppercase tracking-wide px-1.5 py-0.5 rounded-full backdrop-blur-sm ${
                      movie.subscription === "PREMIUM"
                        ? "bg-cyan-500/80 text-white"
                        : "bg-white/20 text-white"
                    }`}
                  >
                    {movie.subscription === "PREMIUM" ? "PRE" : "FREE"}
                  </span>
                </div>
              </div>
