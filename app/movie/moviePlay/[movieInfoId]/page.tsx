...
const Page = () => {
  ...
  useEffect(() => {
    fetch(
      `${process.env.NEXT_PUBLIC_API_URL}/api/movie/movieDetail/${movieInfoId}`,
    )
      .then((r) => r.json())
      .then((data: MovieInfo) => setMovie(data))
  ...
  const embedUrl = movie.videoUrl
    ? `https://iframe.mediadelivery.net/embed/${BUNNY_LIBRARY_ID}/${movie.videoUrl}?defaultQuality=720`
    : null;
